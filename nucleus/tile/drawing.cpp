...
std::vector<tile::Id> generate_list(
    const camera::Definition& camera, utils::AabbDecoratorPtr aabb_decorator, unsigned int max_zoom_level, float error_threshold_px, double planet_radius_m)
{
    const auto tile_refine_functor = tile::utils::refineFunctor(camera, aabb_decorator, 256, max_zoom_level, error_threshold_px, planet_radius_m);
