...
namespace utils {
    ...
    inline auto refineFunctor(const nucleus::camera::Definition& camera, const AabbDecoratorPtr& aabb_decorator, float error_threshold_px, double tile_size = 256, uint32_t max_zoom_level = 18)
    {
        ...
        auto refine = [&camera, camera_frustum, error_threshold_px, tile_size, aabb_decorator, max_zoom_level](const tile::Id& tile) {
            if (tile.zoom_level >= max_zoom_level) // 18)
                return false;
