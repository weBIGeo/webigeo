...
fn should_paint(col: u32, row: u32) -> bool {
    return (col % settings.sampling_interval.x == 0) && (row % settings.sampling_interval.y == 0);
}

@compute @workgroup_size(16, 16, 1)
fn computeMain(@builtin(global_invocation_id) id: vec3<u32>) {
    // id.xy in [0, ceil(texture_dimensions(normals_texture).xy / workgroup_size.yz) - 1]
    ...
    let col = id.x;
    let row = id.y;
    let tex_pos = vec2i(i32(id.x), i32(id.y));
    ...
    if (slope_angle < settings.min_slope_angle || slope_angle > settings.max_slope_angle || !should_paint(col, row)) {
