...
namespace webgpu_engine::sky::lut {
...
std::unique_ptr<SkyAtmosphereLutRenderer> SkyAtmosphereLutRenderer::create(
    WGPUDevice device, webgpu::RenderResourceRegistry& registry, const config::SkyAtmosphereRendererConfig& config)
{
    auto pipelines = pipelines::SkyAtmospherePipelines::create(device, registry, config);
    auto resources = std::make_unique<sky::resources::SkyAtmosphereResources>(device, config);
    ...
    if (config.initializeConstantLuts) {
        ...
        descriptor.label = sv("LUT command encoder");
        webgpu::raii::CommandEncoder encoder(device, descriptor);
        {
            WGPUComputePassDescriptor compute_pass_desc {};
            compute_pass_desc.label = sv("LUT compute pass");
        ...
        cmd_buffer_descriptor.label = sv("LUT command buffer");
