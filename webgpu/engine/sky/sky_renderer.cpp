    ...
    WGPUPipelineLayout pipeline_layout, WGPUShaderModule shader_module, lut::SkyAtmosphereLutRenderer& lut_renderer)
{
    ...
    descriptor.label = sv("Render sky with LUTs pipeline");
    ...
    WGPUDevice device, webgpu::RenderResourceRegistry& registry, config::SkyAtmosphereRendererConfig config)
{
    ...
    registry.register_shader("sky_render_with_luts", "webgpu_engine::sky/render_sky_with_luts");

    auto pipeline = make_compute_pipeline(device, config, pipeline_layout->handle(), registry.shader("sky_render_with_luts").handle(), *lut_renderer);
