...
namespace webgpu_engine::sky::pipelines {
    ...
    WGPUDevice device, WGPUPipelineLayout pipeline_layout, WGPUShaderModule shader_module, uint32_t sample_count)
{
    ...
    descriptor.label = sv("transmittance LUT");
    descriptor.layout = pipeline_layout;
    descriptor.compute = {};
    descriptor.compute.entryPoint = sv("render_transmittance_lut");
...
std::unique_ptr<TransmittanceLutPipeline> TransmittanceLutPipeline::create(
    WGPUDevice device, webgpu::RenderResourceRegistry& registry, WGPUTextureFormat transmittance_lut_format, uint32_t sample_count)
{
    ...
    registry.register_shader("sky_transmittance_lut", "webgpu_engine::sky/render_transmittance_lut");
    auto pipeline = make_compute_pipeline(device, pipeline_layout->handle(), registry.shader("sky_transmittance_lut").handle(), sample_count);
    ...
    WGPUDevice device, WGPUPipelineLayout pipeline_layout, WGPUShaderModule shader_module, uint32_t sample_count)
{
    ...
    WGPUComputePipelineDescriptor descriptor {};
    descriptor.label = sv("multi scattering LUT pass");
    descriptor.layout = pipeline_layout;
    descriptor.compute = {};
    descriptor.compute.entryPoint = sv("render_multi_scattering_lut");
    descriptor.compute.module = shader_module;
    descriptor.compute.constantCount = constants.size();
    descriptor.compute.constants = constants.data();
    return std::make_unique<webgpu::raii::ComputePipeline>(device, descriptor);
...
std::unique_ptr<MultiScatteringLutPipeline> MultiScatteringLutPipeline::create(
    WGPUDevice device, webgpu::RenderResourceRegistry& registry, WGPUTextureFormat multi_scattering_lut_format, uint32_t sample_count)
{
    ...
    registry.register_shader("sky_multi_scattering_lut", "webgpu_engine::sky/render_multi_scattering_lut");
    auto pipeline = make_compute_pipeline(device, pipeline_layout->handle(), registry.shader("sky_multi_scattering_lut").handle(), sample_count);
    ...
    float fromKilometersScaleFactor, bool use_moon, config::MieHgDPhaseConfig miePhaseConfig)
{
    ...
    sky_view_lut_res_y_constant.key = sv("SKY_VIEW_LUT_RES_Y");
    sky_view_lut_res_y_constant.value = double(skyViewLutSize.y);

    WGPUConstantEntry inv_distance_to_max_sample_count_constant {};
    inv_distance_to_max_sample_count_constant.key = sv("INV_DISTANCE_TO_MAX_SAMPLE_COUNT");
    ...
    multi_scattering_lut_res_x_constant.key = sv("MULTI_SCATTERING_LUT_RES_X");
    ...
    for (uint32_t i = 0; i < mie_phase_overrides.size(); i++) {
        constants.push_back(WGPUConstantEntry {
            .nextInChain = nullptr,
            .key = sv(mie_phase_overrides[i].first.c_str()),
            .value = mie_phase_overrides[i].second,
        });
    }

    WGPUComputePipelineDescriptor descriptor {};
    descriptor.label = sv("sky view LUT pass");
    descriptor.layout = pipeline_layout;
    descriptor.compute = {};
    descriptor.compute.entryPoint = sv("render_sky_view_lut");
    descriptor.compute.module = shader_module;
    descriptor.compute.constantCount = constants.size();
    descriptor.compute.constants = constants.data();
    return std::make_unique<webgpu::raii::ComputePipeline>(device, descriptor);
    ...
    float from_kilometers_scale_factor, bool randomize_sample_offsets, bool use_moon, config::MieHgDPhaseConfig mie_phase_config)
{
    ...
    ap_distance_per_slice_constant.key = sv("AP_DISTANCE_PER_SLICE");
    ...
    randomize_sample_offsets_constant.key = sv("RANDOMIZE_SAMPLE_OFFSET");
    ...
    WGPUComputePipelineDescriptor descriptor {};
    descriptor.label = sv("aerial perspective LUT pass");
    descriptor.layout = pipeline_layout;
    descriptor.compute = {};
    descriptor.compute.entryPoint = sv("render_aerial_perspective_lut");
    descriptor.compute.module = shader_module;
    descriptor.compute.constantCount = constants.size();
    descriptor.compute.constants = constants.data();
    return std::make_unique<webgpu::raii::ComputePipeline>(device, descriptor);
...
std::unique_ptr<SkyAtmospherePipelines> SkyAtmospherePipelines::create(
    WGPUDevice device, webgpu::RenderResourceRegistry& registry, config::SkyAtmosphereRendererConfig config)
{
    auto transmittance_lut_pipeline = sky::pipelines::TransmittanceLutPipeline::create(
