    ...
    glm::uvec2 output_size)
{
    ...
    // ToDo: the set  changes when the camera does, properties or the height - It is possible to
    // only record_wanted_tiles in such a scenario but since we actually try to draw only on camera change
    // anyway I think its reasonable to readback every frame. With TAA and other animations implemented it
    // is possible to save around 1/4 ms per frame
    if (record_wanted_tiles && !m_wanted_tiles_map_pending && m_wanted_tiles_staging->map_state() == WGPUBufferMapState_Unmapped) {
        ...
        // The copy is only mapped at the start of draw(), but the window renders on demand. With a
        // still camera that frame might never come and the wanted tiles would never reach.  We ask for
        // one more frame. It will map (so no new copy is requested again) and this can terminate
        if (m_engine_ctx)
