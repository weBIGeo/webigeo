...
// NOTE: Combining with sky pass might be possible and more performant

@group(0) @binding(0) var sky_result: texture_2d<f32>; // full-res RGBA16F output of the sky LUT compute pass
@group(0) @binding(1) var cloud_color: texture_2d<f32>; // full-res RGBA16F upscaled cloud color (rgb=premultiplied radiance, a=transmittance)
@group(0) @binding(2) var composite_out: texture_storage_2d<rgba16float, write>; // write target, same format/size as sky_result
...
fn computeMain(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let tonemapped = straight_rgb / (straight_rgb + 1.0); // Reinhard tone map
