...
@group(0) @binding(1) var input_texture: texture_2d<u32>;
@group(0) @binding(2) var output_texture: texture_storage_2d<rgba8unorm, write>;

fn norm(v: u32) -> f32 {
    return clamp((f32(v) - settings.value_min) / (settings.value_max - settings.value_min), 0.0, 1.0);
}
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    if (settings.num_channels == 1u) {
        ...
        out = vec4f(norm(raw.r), norm(raw.g), norm(raw.b), norm(raw.a));
    }
