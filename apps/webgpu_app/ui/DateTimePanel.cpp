...
void DateTimePanel::ready()
{
    ...
    connect(m_clouds_manager, &clouds::Manager::tileset_list_ready, this, [this]() { recalculate_and_apply(true); });
...
void DateTimePanel::draw()
{
    ...
    if (ImGuiManager::BeginSnapWindow("##datetime_panel", avail,
        ...
        if (!m_cloud_linked) {
            ...
            case CloudLinkState::Unavailable:
                cloud_btn_col = ImVec4(0.4f, 0.4f, 0.4f, 1.0f);
                break;
            case CloudLinkState::Green:
                cloud_btn_col = ImVec4(0.2f, 0.8f, 0.2f, 1.0f);
                break;
            case CloudLinkState::Yellow:
                cloud_btn_col = ImVec4(0.9f, 0.8f, 0.1f, 1.0f);
                break;
            case CloudLinkState::Red:
                cloud_btn_col = ImVec4(0.9f, 0.2f, 0.2f, 1.0f);
                break;
            }
        }
...
void DateTimePanel::recalculate_and_apply(bool load_cloud)
{
    ...
    const auto* best
        = m_clouds_manager->find_best_tileset(utc_dt.date().year(), utc_dt.date().month(), utc_dt.date().day(), utc_dt.time().hour(), utc_dt.time().minute());
    ...
    if (!best) {
        ...
        QDateTime tileset_utc(QDate(best->date.year, best->date.month, best->date.day), QTime(best->date.hour, 0, 0), Qt::UTC);
